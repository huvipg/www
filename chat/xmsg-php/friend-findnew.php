...
{global $conn;
$sql="SELECT * FROM sns_friend where  (friend_frommid='$formid' or friend_tomid='$formid')and (friend_followstate=2 or friend_followstate=1)";
  $result = $conn->query($sql);
                       if ($result->num_rows > 0) {
                          // 输出数据
                              $num=0;
                              $josna= '{"aks":"'.$formid.'", "token":200,"warning":"'.'新的朋友列表请求完成!'.'","data": [';
                              $josnb="";
                          while( $row = $result->fetch_assoc()) {
$a=$row['friend_frommid'];
$b=$row['friend_tomid'];
$friend_followstate=$row["friend_followstate"];
if ($a==$formid ){
  $id=$row['friend_tomid'];
...
$josnb .=   '{"id": "' . $id . '", "name": "' . $name . '","mavata": "' . $tomavata .  '","friend_followstate":'  . $friend_followstate .'},';
}
